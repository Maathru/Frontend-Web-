...
const columns = [
  ...
  { field: "dateScheduled", headerName: "Date Scheduled", flex: 1, valueGetter: FormattedDate },
  { field: "dateAdministered", headerName: "Date Administered", flex: 1, valueGetter: FormattedDate },
  {
    field: "status",
    headerName: "Status",
    flex: 1,
    renderCell: (params) => (
      <YesNoButton
        ...
        onChange={(newValue) => {
          params.api.updateRows([{ id: params.id, status: newValue ? "Administered" : "Pending" }]);
        }}
        disabled={true}
      />
    ),
...
const sampleRecord = [
      {
        ...
        status: "Administered",
        details: "Protects against tuberculosis using a weakened strain of Mycobacterium bovis",
      },
      {
        id: 2,
              ...
              <div key={rowIndex} className="flex space-x-1 w-full">
                {row.map((detail, index) => (
                  <div key={index} className="flex-1 flex space-x-2">
                    <CardDescription className="font-bold">{detail.label}:</CardDescription>
